...
class Fastq(Fasta):
    ...
    def __str__(self):
        ...
        return "@{0}\n{1}\n+\n{2}".format(self.id, self.seq, len(self.qual) == self.length() and self.qual or ''.join(
...
class TabixFile(object):
    ...
    def BaitStatsPlot(self, bait, outfile, extendsize=100000, readlen=36, smooth_window=100):
        ...
        #ALICIA - COMMENTED OUT BROKEN PLOTTING CODE
        #import seaborn as sns
        #import matplotlib.pyplot as plt
        #ns.set_context('poster')
        #ns.set_style('ticks')
        #ig = plt.figure(figsize=[10, 10])
        #x1 = plt.subplot2grid((20, 10), (0, 0), rowspan=10, colspan=4)
    ...
    def GetIntraChromLinks(self, outfile=None, nbins=11, nperm=10000, seed=1024):
        ...
        for column in cdf.columns:
            n, p = Algorithms.NBFit(list(cdf[column]))
            ns = numpy.append(ns, n)
            ps = numpy.append(ps, p)
        ps = ps[1:]
        ns = ns[1:]
        #TO HERE TO REPLACE THE FOLLOWING LINE:
    ...
    def GetInterChromLinks(self, outfile=None, binsize=1000000, nperm=10000, seed=1024):
        ...
        '''
        chroms, sizes = zip(*[(chrom, size) for chrom, size in zip(self.chroms, self.sizes) if
                              chrom != self.bait_chrom and size > binsize])
        ...
        with open(outprefix + "_wu.bedpairs", 'w') as ofh:
            for chr1, start1, chr2, start2, idx in read_pairs:
                if chr1 == chr2:  # intra
                    ...
                    p = 1 - stats.nbinom.cdf(intra_counts[idx], self.intra_nb.loc[pcname, 'r'],
                                             self.intra_nb.loc[pcname, 'p'])
                    print >> ofh, "{0}:{1}-{2}\t{3}:{4}-{5}\t{6:.0f}".format(chr1, start1, int(start1) + self.rlen,
                                                                             chr2, start2, int(start2) + self.rlen,
                                                                             min(150, numpy.ceil((1 - p) / p / 1000)))
                elif chr2 in self.inter_ns:  # inter
                    p = 1 - stats.nbinom.cdf(inter_counts[chr2][idx], self.inter_ns[chr2], self.inter_ps[chr2])
                    print >> ofh, "{0}:{1}-{2}\t{3}:{4}-{5}\t{6:.0f}".format(chr1, start1, int(start1) + self.rlen,
                                                                             chr2, start2, int(start2) + self.rlen,
                                                                             -min(150, numpy.ceil((1 - p) / p / 100)))
...
'''.format(genome=genome,
    ...
    def _RmDup3():
        ...
        for line in sys.stdin:
            ...
            if items[2] != curchr or items[3] != curpos:
                for read in reads:
                    print
                    "{0}\t{1}\t{2}".format(curchr, curpos, read)
            ...
            for curpos in sorted(pairs[curchr]):
                for chrom in sorted(pairs[curchr][curpos]):
                    for pos in sorted(pairs[curchr][curpos][chrom]):
                        print
                        "{0}\t{1}\t{2}\t{3}".format(curchr, curpos, chrom, pos)
    ...
    def FixMatePairs(bams, prefix, nproc=1, overwrite=False):
        ...
        if overwrite or not (os.path.isfile(tbffile) or os.path.isfile(tbffile + ".gz")):
            ...
            # Remove duplicates
            cmd = '''samtools merge -nf -h {0} - {1} |samtools fixmate -pr - - |samtools view -h -f 1 -|samtools sort -@ {2} -O SAM -T {3} -|python -c "import c3s;c3s.Algorithms._RmDup3()" >{3}.pairs'''.format(
                bams[0], " ".join(bams), max(nproc - 3, 1), prefix)
            Utils.touchtime(cmd)
            rc = call(cmd, shell=True)
            if rc != 0:
...
class Plot(object):
    ...
    def BaitCountPie(cnts, labels, ax):
        ...
        '''
        import matplotlib.pyplot as plt
        ax.pie(cnts, labels=None, colors=['grey', 'blue', 'red'], shadow=False, startangle=60, labeldistance=0.8,
               pctdistance=0.45, textprops={'ha': 'left'})
